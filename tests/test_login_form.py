...
def test_login_form(browser):
    ...
    form_page.current_address.send_keys('Elms street')


def test_state_2(browser):                #выбрать элемент из выпадающего списка по локатору
    form_page = FormPage(browser)

    form_page.visit()                   #1 вариант
    time.sleep(2)
...
def test_state_3(browser):          # 2 вариант
    form_page = FormPage(browser)
...
def test_state_1(browser):          # 3 вариант
    form_page = FormPage(browser)
...
#   self.btn_state = WEmElement(driver, #state)
